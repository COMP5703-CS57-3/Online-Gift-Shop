...
class User(database.Model):
    ...
    user_name = database.Column(database.String(120))
    user_email = database.Column(database.String(120), unique=True)
    user_password = database.Column(database.String(60))
    user_mobile = database.Column(database.String(60))
    user_date_of_birth = database.Column(database.String(60))
    user_address = database.Column(database.String(150))
...
class ValidationInformation(database.Model):
    ...
    id = database.Column(database.Integer, unique=True, primary_key=True)
    user_email = database.Column(database.String(120), database.ForeignKey('user.user_email', ondelete='CASCADE'))
    validation_code = database.Column(database.String(60))

class Admin(database.Model):
    __tablename__ = 'admin'
    id = database.Column(database.Integer, unique=True, primary_key=True)
    admin_email = database.Column(database.String(120), unique=True)
    admin_name = database.Column(database.String(120))
    admin_password = database.Column(database.String(120))
    admin_mobile = database.Column(database.String(60))

class Gifts(database.Model):
    __tablename__ = 'gifts'
    id = database.Column(database.Integer, unique=True, primary_key=True)
    gift_name = database.Column(database.String(200))
    gift_price = database.Column(database.Float)
    gift_discount_price = database.Column(database.Float)
    gift_discount_state = database.Column(database.String(60), default='no')
    gift_description = database.Column(database.String(500))
    gift_category = database.Column(database.String(120))
...
class Size(database.Model):
    ...
    size = database.Column(database.String(120))
...
class Cart(database.Model):
    ...
    gift_size = database.Column(database.String(120))


class Order(database.Model):
    __tablename__ = 'order'
    id = database.Column(database.Integer, unique=True, primary_key=True)
    order_time = database.Column(database.String(120))
    order_total = database.Column(database.Float)
    order_number = database.Column(database.String(120))
    first_name = database.Column(database.String(120))
    last_name = database.Column(database.String(120))
    phone = database.Column(database.String(60))
    address = database.Column(database.String(500))
    postcode = database.Column(database.String(60))
    user_id = database.Column(database.Integer, database.ForeignKey('user.id', ondelete='CASCADE'))
    # setting relationship
...
class OrderItems(database.Model):
    ...
    id = database.Column(database.Integer, unique=True, primary_key=True)
    gift_name = database.Column(database.String(120))
